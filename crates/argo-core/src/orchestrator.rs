...
impl OrchestratorActor {
    ...
    async fn decompose_goal(
        ...
        let sub_tasks: Vec<String> = serde_json::from_str(&response.content)
            .unwrap_or_else(|_| vec![goal.to_string()]);
        Ok(sub_tasks)
    }
...
impl OrchestratorActor {
    pub fn decompose_and_assign(
        ...
        let fut = async move {
            Self::decompose_goal(llm.as_ref(), &goal_clone).await
        }
        .into_actor(self)
            ...
            match result {
                Ok(sub_goals) => {
                    ...
                    for (task_id, sub_goal) in sub_tasks {
                        ...
                        ctx.spawn(
                            worker_fut.into_actor(actor).map(move |result, actor, _ctx| {
                                ...
                                actor.results.insert(task_id, result.unwrap_or_else(|e| {
                                    TaskResult::Failed { error: e }
                                }));
                            }),
                        );
                    }
